...
export default React.memo(function AnalysisResult({ data }: { data: GeminiFloodAnalysis }) {
  return (
    <motion.div
      variants={container}
      initial="hidden"
    ...
    >
      ...
      >
        <div>
          <p className="text-xs text-slate-300 uppercase tracking-widest mb-1 font-mono">
            JalPath Analysis
          </p>
      ...
      <div className="p-5 space-y-5">
        ...
        <motion.div variants={item} className="grid grid-cols-2 gap-3">
          <div className="bg-slate-900/60 rounded-xl p-4 flex items-start gap-3">
            ...
            <div>
              <p className="text-xs text-slate-300 mb-0.5">Water Depth</p>
          ...
          <div className="bg-slate-900/60 rounded-xl p-4 flex items-start gap-3">
            <Gauge className="w-5 h-5 text-slate-300 mt-0.5 shrink-0" aria-hidden="true" />
            <div>
              <p className="text-xs text-slate-300 mb-0.5">AI Confidence</p>
        ...
        {data.riskFactors.length > 0 && (
          <motion.div variants={item}>
            <h3 className="text-xs text-slate-300 uppercase tracking-widest mb-2 flex items-center gap-1.5">
        ...
        {/* Escape actions */}
        <motion.div variants={item}>
          <h3 className="text-xs text-slate-300 uppercase tracking-widest mb-3 flex items-center gap-1.5">
            ...
            {data.escapeActions.map((action, i) => (
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-300 mt-0.5">{action.detail}</p>
                </div>
              </motion.li>
            ))}
          </ol>
